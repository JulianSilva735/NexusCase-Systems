  ...
  return (
    <Paper sx={{ p: 1.5, borderRadius: 2, border: '1px solid #e2e8f0' }}>
      <Stack direction="row" spacing={1} alignItems="center" sx={{ mb: 1.5 }}>
        ...
        <Chip
          size="small"
          color={lifecycle.source === 'MANUAL' ? 'warning' : 'success'}
          label={`Fuente: ${lifecycle.source}`}
        />
        <Chip size="small" variant="outlined" label={`Etapa actual: ${lifecycle.current_stage_label}`} />
      </Stack>

      {lifecycle.manual_override.active && (
        <Alert severity="warning" sx={{ mb: 1.5 }}>
          Override manual activo: {lifecycle.manual_override.target_stage || 'N/D'}
          {lifecycle.manual_override.reason ? ` | Motivo: ${lifecycle.manual_override.reason}` : ''}
          {lifecycle.manual_override.user_id ? ` | Usuario: ${lifecycle.manual_override.user_id}` : ''}
          {lifecycle.manual_override.changed_at
            ? ` | Fecha: ${new Date(lifecycle.manual_override.changed_at).toLocaleString()}`
            : ''}
        </Alert>
      )}
